...
public class PreemptiveFrameFormationConsumer64 extends ArchFormationConsumer<FormationFrame>
        implements PreemptiveFrameFormationConsumer {
    protected FormationStrategyRuntime mRuntime;
    protected MasterManipulator        mMasterManipulator;
    protected GUID                     mRunGuid;

    public PreemptiveFrameFormationConsumer64(
    ...
    @Override
    public PreemptiveFrameFormationProducer producer() {
        return (PreemptiveFrameFormationProducer)this.mProducer;
    }
    ...
    public void consume() {
        while ( this.mProducer.hasMoreProducts() ) {
            ...
            FormationFrame frame = this.mProducer.require();
            if ( frame == null ) {
                break;
            }
            ...
            try {
                ...
                this.mProducer.deactivate( frame );
            }
        }
    }

    @Override
    public void consume( Frame unit ) {
        if ( unit instanceof FormationFrame ) {
            this.consumeFrame( (FormationFrame)unit );
        }
    }
