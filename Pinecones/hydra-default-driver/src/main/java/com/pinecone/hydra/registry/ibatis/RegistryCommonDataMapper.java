...
public interface RegistryCommonDataMapper extends RegistryCommonDataManipulator {
    @Insert("INSERT INTO `hydra_registry_node_common_data` (`guid`) VALUES (#{guid})")
    void insert(NodeCommonData nodeCommonData);

    @Delete("DELETE FROM `hydra_registry_node_common_data` WHERE `guid`=#{guid}")
    void remove(GUID guid);

    @Select("SELECT `id`, `guid` FROM `hydra_registry_node_common_data` WHERE `guid`=#{guid}")
