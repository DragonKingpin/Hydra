...
import com.pinecone.hydra.device.kom.entity.FolderElement;
...
public class DeviceJSONDecoder implements DeviceInstrumentDecoder {
    protected DeviceInstrument instrument;

    public DeviceJSONDecoder(DeviceInstrument instrument ) {
    ...
    public ElementNode    decode( String szName, Object o, GUID parentGuid ) {
        ...
        throw new IllegalArgumentException( "Elements of `DeviceInstrument` should all be object." );
    }
                ...
                for( ElementNode node : destChildren ) {
                    if( szName.equals( node.getName() ) ) {
                        if( node instanceof Namespace ) {
                            ...
                            throw new IllegalArgumentException(
                                    String.format( "<DeviceInstrument> Existed child-destination [%s] should be namespace.", szName )
                            );
                        }
                    }
                }
    ...
    protected ElementNode decodeJSONObject( String szName, Map<String, Object > jo, GUID parentGuid ) {
        ...
        if ( isNamespace ) {
            ...
            if( bIsFolderElement ) {
                Object services = jo.get( "devicements" );
