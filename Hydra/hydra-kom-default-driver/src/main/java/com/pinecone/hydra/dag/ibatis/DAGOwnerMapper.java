...
public interface DAGOwnerMapper extends LayerOwnerManipulator {
    @Insert("INSERT INTO `hydra_vgraph_dag_tree` (`guid`) VALUES ( #{guid} )")
    void insertRootNode(@Param("guid") GUID guid );

    @Insert( "INSERT INTO `hydra_vgraph_dag_tree` (`guid`, `parent_guid`) VALUES (#{targetGuid}, #{parentGuid})" )
    void insert( @Param("targetGuid") GUID targetGuid, @Param("parentGuid") GUID parentGUID );


    @Update( "UPDATE `hydra_vgraph_dag_tree` SET `guid` = #{targetGuid}, `parent_guid` = #{parentGuid} WHERE `guid` = #{targetGuid}" )
    ...
    @Delete( "DELETE FROM `hydra_vgraph_dag_tree` WHERE `guid`=#{subordinateGuid} " )
