...
public interface TaskMetaMapper extends TaskMetaManipulator {
    ...
    @Select("SELECT `id` AS `enumId`, `guid`, `name`, `path`, `type`, `alias`, `resource_type` AS resourceType, `service_type` AS serviceType, `create_time` AS createTime, `update_time` AS updateTime FROM `hydra_task_serv_node_meta` WHERE `guid`=#{guid}")
