...
public class ServiceLegionaryExpiredDevilCase implements Pinenut {

    protected Tritium mSystem;

    protected ServiceLegionarySmokeCase mSmokeCase;

    public ServiceLegionaryExpiredDevilCase( Tritium system ) {
        this.mSystem = system;
    ...
    public void run( ServiceLegionaryTransportScenario scenario ) throws Exception {
        ...
        try {
            ...
            context.probe.record( "ExpiredDevil", 0, "INITIAL_ONLINE", context.legionary );
            ServiceLegionaryAssertions.assertOnline( context.legionary );

            GUID instanceGuid = context.legionary.getInstanceGuid();
            context.probe.record( "ExpiredDevil", 1, "BREAK", context.legionary );
            ...
            context.probe.record( "ExpiredDevil", 1, "DETACHED", context.legionary );

            ServiceLegionaryAssertions.awaitTrue(
                    context.scenario.name() + " did not expire.",
                    10000L,
                    () -> this.hasStatus( context, instanceGuid, ServiceInstanceStatus.Expired )
                            && !context.serviceManager.hasOwnedInstance( instanceGuid )
            );
            context.probe.record( "ExpiredDevil", 1, "EXPIRED", context.legionary );
            Debug.greenfs( "[ServiceLegionaryExpiredDevil] " + scenario.name() + " PASS" );
        }
        finally {
            context.probe.dump();
            context.cleanup();
    ...
    protected ServiceLegionarySmokeContext createContext( ServiceLegionaryTransportScenario scenario ) throws Exception {
        ...
        context.detachedObservationConfig = new ServiceDetachedObservationConfig(
                new JSONMaptron( "{enable:true, graceMillis:300, sweepMillis:50, expireAsyncThreads:2, missingAfterReconnectPolicy:\"Expired\"}" )
        );
