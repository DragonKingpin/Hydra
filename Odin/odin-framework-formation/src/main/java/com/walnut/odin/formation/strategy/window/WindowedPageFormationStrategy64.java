...
public class WindowedPageFormationStrategy64 implements WindowedPageFormationStrategy {
    protected GUID                         mRunGuid;
    protected GuidAllocator                mGuidAllocator;
    protected MasterManipulator            mMasterManipulator;
    protected String                       mszClaimOwner;
    protected WindowedPageFormationProducer mProducer;

    public WindowedPageFormationStrategy64(
            GUID runGuid,
    ...
    public FormationStrategyType kind() {
        return FormationStrategyType.WindowedPage;
    }
    ...
    @Override
    public WindowedPageFormationProducer formulateProducer( FormationStrategyContext context, FormationStrategyRuntime runtime ) {
        if ( this.mProducer != null ) {
            return this.mProducer;
        }
        this.mProducer = new WindowedPageFormationProducer64(
                context,
                runtime,
                this.mRunGuid,
                this.mGuidAllocator,
    ...
    @Override
    public WindowedPageFormationConsumer formulateConsumer( FormationStrategyContext context, FormationStrategyRuntime runtime ) {
        FormationFrameConsumerAdapter adapter = new UniformFormationFrameConsumerAdapter( context.taskScheduler() );
        return new WindowedPageFormationConsumer64(
